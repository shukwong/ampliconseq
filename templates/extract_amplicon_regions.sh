...
JAVA_OPTS="-Xmx!{java_mem}m" extract-amplicon-regions \
    ...
    !{params.maximumReadsPerAmplicon > 0 ? "--maximum-reads-per-amplicon ${params.maximumReadsPerAmplicon}" : ''} !{params.downsamplePercentile > 0 ? "--downsample-percentile ${params.downsamplePercentile}" : ''}
